...
from baserow_dynamic_table.core.utils import ChildProgressBuilder, exception_capturer
...
from baserow_dynamic_table.search.expressions import LocalisedSearchVector
from baserow_dynamic_table.search.regexes import (
...
if TYPE_CHECKING:
    ...
    from baserow_dynamic_table.table.handler import TableForUpdate

    from baserow_dynamic_table.table.models import (
        GeneratedTableModel,
        Table,
    )
...
class SearchHandler:
    ...
    def special_char_tokenizer(cls, expression: Expression) -> Func:
        ...
        Emails:
          With input "peter@baserow_dynamic_table.com" this will result in tokens:
          1. peter
          2. baserow_dynamic_table.io
        URLs
          With input "https://baserow_dynamic_table.io/jobs/" this will result in tokens:
          1. https
          2. baserow_dynamic_table.io
          3. jobs
        Dates
    ...
    def escape_postgres_query(cls, text, per_token_wildcard: bool = False) -> str:
        ...
        :param per_token_wildcard: Determines whether we add the `:*` wildcard to
            ...
            but is problematic for baserow_dynamic_table's frontend, so we only add the wildcard at
    ...
    def after_field_created(cls, field: "Field", skip_search_updates: bool = False):
        """
        :param field: The baserow_dynamic_table field which was created in this table.
    ...
    def after_field_perm_delete(
        ...
        """
        :param field: The current baserow_dynamic_table field which was deleted from this table.
        :return: None
        """
    ...
    def try_slower_but_best_effort_tsv_update(
        ...
        """
        Given the complexity of all possible field configurations in baserow_dynamic_table there is
    ...
    def _trigger_async_tsvector_task_if_needed(
        ...
        if table.tsvectors_are_supported:
            from baserow_dynamic_table.search.tasks import (
                async_update_tsvector_columns,
            )
            from baserow_dynamic_table.tasks import (
